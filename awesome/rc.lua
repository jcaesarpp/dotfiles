...
local notification_calendar
local widget_weather = require("awesome-wm-widgets.weather-widget.weather")
...
--local notify_brightness
--local function brightness_down()
--	if notify_brightness == nil then
--		notify_brightness = naughty.notify{
--			text = 'text to display',
--			timeout = 3,
--			width = auto,
--			destroy = function() notify_brightness = nil end
--		}
--	else
--		naughty.destroy(notify_brightness)
--	end
--end
...
widget_clock = wibox.widget.textclock("%R", 60, "UTC-03")
...
widget_clock:connect_signal("button::release",
	function()
		if notification_calendar == nil then
			awful.spawn.easy_async([[bash -c "cal -3 | sed 's/_.\(.\)/+\1-/g'"]],
				function(stdout, stderr, reason, exit_code)
					notification_calendar = naughty.notify{
						text = string.gsub(string.gsub(stdout, "+",
							"<span foreground='red'>"), "-", "</span>"),
						timeout = 0,
						width = auto,
						destroy = function() notification_calendar = nil end
					}
				end
			)
		else
			naughty.destroy(notification_calendar)
		end
	end)
